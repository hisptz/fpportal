...
FPServices.factory('FPManager',function($http,$q,portalService,$location){
    var FPManager = {
    ...
    var period = "201803";
    var periodName = "March 2018"
    if($location.search().period){
        period = $location.search().period;
        periodName = FPManager.getMonthName($location.search().period);
        FPManager.lastMonthWithData = period
        FPManager.lastMonthWithDataName = periodName;
        FPManager.latestYear = period.substr(0,4);
        FPManager.latestMonth = period;
    }
    return FPManager;
});
