            ...
            }else
            {
                $.post( portalService.base + "dhis-web-commons-security/login.action?authOnly=true", {
                    ...
                    $http.get('FPFacilities.json').success(function(data){
                        ...
                        $http.get(portalService.base+'api/sqlViews/bUYj0dQ4RUE/data.json?var=types:Hospital&var=methods:'+method+'&var=year:2016').success(function(val1){
                            $rootScope.showProgressMessage = false;
                        ...
                        $http.get(portalService.base+'api/sqlViews/bUYj0dQ4RUE/data.json?var=types:Health Center&var=methods:'+method+'&var=year:2016').success(function(val1){
                            $rootScope.showProgressMessage = false;
                            angular.forEach(orgUnits, function (yAxis) {
                                var serie = [];
                                angular.forEach(periods, function (xAxis) {
                        ...
                        $http.get(portalService.base+'api/sqlViews/bUYj0dQ4RUE/data.json?var=types:Dispensary&var=methods:'+method+'&var=year:2016').success(function(val1){
                            $rootScope.showProgressMessage = false;
                            angular.forEach(orgUnits, function (yAxis) {
                                var serie = [];
                                angular.forEach(periods, function (xAxis) {
